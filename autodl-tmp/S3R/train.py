...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(npy_path):
            # original behavior: load precomputed feature .npy
            x = np.load(npy_path)
        else:
            # try to load image (file) or first image in directory
            img_path = None
            if os.path.isfile(path):
                img_path = path
            elif os.path.isdir(path):
                # find common image extensions
                files = sorted([f for f in os.listdir(path) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff'))])
                if len(files) > 0:
                    img_path = os.path.join(path, files[0])
            else:
                # try common image extensions appended to path
            ...
            if img_path is None:
                raise FileNotFoundError(f"Neither {npy_path} nor an image file/directory found for path '{path}'")

            # load image, convert to grayscale, resize to (width=self.size, height=self.time)
            img = Image.open(img_path).convert('L')
            # PIL size is (width, height)
            img = img.resize((self.size, self.time), Image.BILINEAR)
            x = np.array(img, dtype=np.float32)
            # normalize to 0-1
            if x.max() > 1.0:
                x = x / 255.0

        # resize feature into (time x size)
        x = x[0:self.time, :]
        x = x[:, 0:self.size]
