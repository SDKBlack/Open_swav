...
class WTNet(nn.Module):
    ...
    def forward_backbone(self, x, return_intermediate_features: bool = True):
        # Position Branch
        pos_feat = None
        ...
        # Capture intermediate features for Stage 2 Clustering (optional)
    ...
    def forward(self, inputs, labels=None, return_intermediate_features: bool = True):
        # Control whether we compute/return intermediate features (used for stage-2 clustering).
        if not isinstance(inputs, list):
            inputs = [inputs]

        # Optimization: for single input (common in inference), skip grouping logic
        # This branch is static-shape friendly (no unique_consecutive) and works for torch.compile
        if len(inputs) == 1:
            out, aux_logits, intermediate_features = self.forward_backbone(
                inputs[0], 
                return_intermediate_features=return_intermediate_features
            )
            # Match multi-crop return signature so eval_openset can reliably
            # consume (embedding, proto_out, [logits/aux], intermediate).
            embedding, proto_out = self.forward_head(out)

            # Keep behavior consistent with the main forward: cache backbone feats
            # for optional losses (e.g., boundary loss).
            self._last_backbone = out

            logits = None
            if self.classifier is not None:
                if labels is not None:
                    logits = self.classifier(out, labels)
                else:
                    logits = self.classifier(out)

            # In single-input mode, intermediate_features is a list of tensors (branches/layers).
            # For stage2 clustering we want a single tensor [B, C_total].
            final_intermediate = None
            if return_intermediate_features and intermediate_features is not None:
                try:
                    if isinstance(intermediate_features, (list, tuple)):
                        final_intermediate = torch.cat(list(intermediate_features), dim=1)
                    else:
                        final_intermediate = intermediate_features
                except Exception:
                    final_intermediate = None

            if logits is not None:
                if self.use_aux_heads:
                    return embedding, proto_out, logits, aux_logits, final_intermediate
                return embedding, proto_out, logits, final_intermediate

            if self.use_aux_heads:
                return embedding, proto_out, aux_logits, final_intermediate
            return embedding, proto_out, final_intermediate

        # Original logic for multi-crop (training) with dynamic grouping
        ...
        start_idx = 0
        aux_logits_list = []
        intermediate_features_list = [] if return_intermediate_features else None
        for end_idx in idx_crops:

            _out, _aux, _inter = self.forward_backbone(
            ...
            if return_intermediate_features and _inter is not None:
        ...
        final_intermediate = None
        if return_intermediate_features and intermediate_features_list:
