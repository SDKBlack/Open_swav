...
class WTConv2d(nn.Module):
    ...
    def _fast_haar_transform(self, x: torch.Tensor) -> torch.Tensor:
        """
        Fast Haar (db1) 2D forward transform implemented with slicing and add/sub operations.
        Input: x [B, C, H, W]
        Output: coeffs [B, C, 4, H//2, W//2] in order [LL, LH, HL, HH]
        """
        # assume H and W are divisible by 2 (padding handled by caller)
        ...
        # combine into subbands; apply normalization consistent with pywt db1 orthonormal
        ll = (r0_sum + r1_sum) * 0.5
        lh = (r0_sum - r1_sum) * 0.5
        hl = (r0_diff + r1_diff) * 0.5
        hh = (r0_diff - r1_diff) * 0.5
